...
async def advantage_spoll_choker(bot, query):
    ...
    if not movies:
        return await query.answer(script.OLD_ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    if int(user) != 6 and query.from_user.id != int(user):
        return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    ...
    if gl == False:
        ...
        if k == False:
            files, offset, total_results = await get_search_results(query.message.chat.id, movie, offset=6, filter=True)
            if files:
    ...
    if query.data.startswith("file"):
                    ...
                    if IS_VERIFY and not await check_verification(client, query.from_user.id):
                        ...
                        await client.send_cached_media(
                            ...
                            reply_markup=InlineKeyboardMarkup(
                                ...
                                InlineKeyboardButton("Bot Owner", url="t.me/TgxKiller")
                                ]
                                ]
                            )
                        )
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    ...
                    InlineKeyboardButton("Owner", url="t.me/TgxKiller")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def auto_filter(client, msg, spoll=False):
    ...
    btn.insert(0, [
        InlineKeyboardButton("! SEND ALL FILES !", callback_data=f"send_fall#{pre}#{0}#{message.from_user.id}"),
